...
export interface paths {
    ...
    "/clients": {
        ...
        /** Clients list (consultancy-side). Default order (2026-09-10) is unallocated clients first, then those with a consultant but no plan, then everyone else, newest client first (created_at desc) within each tier; an explicit sort= replaces all of that. id always appended as the deterministic secondary key (TRD Section 7). sort= accepts name, created_at, last_message_at, status, progress, consultant_name. filter[x]= accepts assigned_to_me=true, unattended=true, unassigned=true (2026-09-10 — clients with no consultant yet; what the dashboard's Pending Consultant Allocation card counts, so the card deep-links to exactly its own rows), tag=<name>[,<name>…] (ANY of the listed; comma-separated, 2026-09-10), show_closed=true (user-requested, 2026-08-15 — closed, closed_switched, closed_completed, and plan_complete clients are excluded by default regardless of any other filter; this is the only way to see them again, same idiom GET /leads's show_closed already uses), destination=<name>[,<name>…] (2026-09-10 — ANY of the student's preferred destinations, `preferences.target_countries`; what the Clients list's Preferred Destination filter sends), country=<name>[,<name>…] (ANY of the listed; comma-separated, 2026-09-10) (user-requested, 2026-08-19 — filters on `Client.finalized_country`, exact match). search matches the applicant's name, email, phone, or file_number (user-requested, 2026-08-15). */
        get: {
            parameters: {
                query?: {
                    /** @description Opaque pagination cursor from a previous response's next_cursor. Omit for the first page. */
...
export interface components {
    schemas: {
        ...
        Money: {
            ...
            /** @example INR */
            currency: string;
            /** @description The same amount in the VIEWER's own currency (2026-09-10), converted server-side with the platform exchange-rate table. The money itself is always the currency it is charged or collected in; this is the "≈" companion. Null when the viewer already reads that currency, when there is no rate, or on responses that do not carry it. */
            readonly approx?: components["schemas"]["ApproxMoney"] | null;
        };
        /** @description An approximate conversion. Rates are set by hand, not live, so this is a guide rather than a quote. */
        ApproxMoney: {
            /** Format: double */
            amount: number;
            /** @example THB */
        ...
        Consultancy: {
            ...
            /** @description The currency this consultancy's staff read money in (2026-09-10) — the default currency of its country from the Countries list, the same rule students and invoices follow. Fee filters are typed in it, and amounts in other currencies carry an `approx` in it. */
            readonly display_currency?: string | null;
        ...
        Course: {
            ...
            /**
             * @description The tuition fee expressed in the VIEWER's own currency (Phase D, 2026-08-22; every viewer since 2026-09-10) — a student's `display_currency`, a consultancy's `display_currency` (its country's default), INR for platform staff. Decorated per request because the currency belongs to the viewer, not to the course.
